...
class N_keterampilan extends CI_Controller {
	function __construct() {
        ...
        $this->d['tahun'] = substr($this->d['tasm'], 0, 4);
         $this->d['tapel'] = (substr($this->d['tasm'], 0, 4))."/".(substr($this->d['tasm'], 0, 4)+1);
    ...
    public function cetak($bawa,$tasm=0) {
        ...
        */
        ...
        $html = '<p align="left"><b>REKAP NILAI KETERAMPILAN</b>
                <br>
                Mata Pelajaran : '.$detil_guru['nmmapel'].', Kelas : '.$detil_guru['nmkelas'].', Guru : '.$detil_guru['nmguru'].', Tahun Pelajaran: '.$this->d['tapel'].'<hr style="border: solid 1px #000; margin-top: -10px"></p>
        ...
        if (!empty($d_nilai)) {
            ...
            foreach ($d_nilai as $ke => $dn) {
                ...
                $sangat_baik = empty($array_sangat_baik);
