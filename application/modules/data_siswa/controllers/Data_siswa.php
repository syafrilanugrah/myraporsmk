...
class Data_siswa extends CI_Controller {
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT m_siswa.id FROM m_siswa JOIN m_jurusan ON m_siswa.jurusan_id=m_jurusan.id WHERE m_siswa.nama LIKE '%".$search['value']."%' AND stat_data = 'A'")->num_rows();
    
        $q_datanya = $this->db->query("SELECT a.*, a.id as id_mhs, c.*,
        ...
        foreach ($q_datanya as $d) {
            ...
            $link_aktif_user = $d['jml_aktif'] > 0 ? '<a href="#" onclick="return nonaktifkan(\''.$d['id_mhs'].'\');" class="btn btn-xs btn-warning"><i class="fa fa-user"></i></a>' : '<a href="#" onclick="return aktifkan(\''.$d['id_mhs'].'\');" class="btn btn-xs btn-info"><i class="fa fa-user"></i></a>';

            $data_ok[4] = '<a href="'.base_url().$this->d['url'].'/edit/'.$d['id_mhs'].'" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a>
                '.$link_aktif_user.'
                <a href="#" onclick="return hapus(\''.$d['id_mhs'].'\');" class="btn btn-xs btn-danger"><i class="fa fa-remove"></i></a> ';
    ...
    public function edit($id) {
        ...
        $jurusan = $this->db->query("SELECT * from m_jurusan")->result_array();
        $arr_jurusan = array(""=>"Pilih");
        foreach ($jurusan as $key => $value) {
            $arr_jurusan[$value["id"]]=$value["jurusan"];
        }
        $this->d['p_jurusan']=$arr_jurusan;
		...
		$this->d['p_diterima_smt']  = array(""=>"Semester","Ganjil"=>"Ganjil","Genap"=>"Genap");
    ...
    public function simpan() {
        ...
        $data['jurusan_id'] = $p['jurusan'];
        $data['diterima_tgl'] = $p['diterima_tgl'];
        $data['diterima_smt'] = $p['diterima_smt'];
	...
	public function nonaktifkan($id) {
        ...
        $detil_data = $this->db->query("SELECT * FROM m_siswa WHERE id = '".$id."'")->row_array();

        if (empty($detil_data)) {
            $d['status'] = "gagal";
            $d['data'] = "Terjadi kesalahan sistem..";
        } else {
            $username = strtolower(str_replace(array(".",","," "), array("","",""), $detil_data['nis']));
            $password = sha1(sha1($username));

            $this->db->query("DELETE FROM m_admin WHERE level = 'siswa' AND konid = '$id'");
    ...
    public function import_siswa() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($b = 2; $b < 500; $b++) {
                ...
                $jurusan_id = $_sheet->getCell('AE'.$b)->getCalculatedValue();

                if ($nis != "" || $nisn != "" || $nama != "") {
                    $data[] = "('$nis', '$nisn', '$nama', '$jk', '$tmp_lahir', '$tgl_lahir', '$agama', '$status', '$anakke', '$alamat', '$notelp', '$sek_asal', '$sek_asal_alamat', '$diterima_kelas', '$diterima_tgl', '$diterima_smt', '$ijazah_no', '$ijazah_thn', '$skhun_no', '$skhun_thn', '$ortu_ayah', '$ortu_ibu', '$ortu_alamat', '$ortu_notelp', '$ortu_ayah_pkj', '$ortu_ibu_pkj', '$wali', '$wali_alamat', '$notelp_rumah', '$wali_pkj', '$jurusan_id')";
                    $no++;
                }
            }
