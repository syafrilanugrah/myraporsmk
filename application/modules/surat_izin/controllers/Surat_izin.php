...
class Surat_izin extends CI_Controller {
    ...
    public function cetak($id_siswa,$tasm) {
        ...
        $kelompok = array("A", "B","C");
            ...
            $d['nilai_utama'] .= '<tr><td colspan="7"><b>KELOMPOK A</b></td></tr>';
            $no = 1;
            ...
            foreach ($q_mapel as $i=>$m) {
                ...
                $nkd = empty($nilai_keterampilan[$idx]['desk']) ? "-" : "Capaian kompetensi sudah tuntas dengan predikat ".nilai_pre($npa);
            ...
            //kelompok B
            $d['nilai_utama'] .= '<tr><td colspan="7"><b>KELOMPOK B</b></td></tr>';
            ...
            foreach ($q_mapel as $i=>$m) {
                ...
                $npp = empty($nilai_pengetahuan[$idx]['predikat']) ? "-" : $nilai_pengetahuan[$idx]['predikat'];
                $npd = empty($nilai_pengetahuan[$idx]['desk']) ? "-" : "Capaian kompetensi sudah tuntas dengan predikat ".nilai_pre($npa);
                $nka = empty($nilai_keterampilan[$idx]['nilai']) ? "-" : $nilai_keterampilan[$idx]['nilai'];
                $nkp = empty($nilai_keterampilan[$idx]['predikat']) ? "-" : $nilai_keterampilan[$idx]['predikat'];
            ...
            //kelompok C
            $d['nilai_utama'] .= '<tr><td colspan="7"><b>KELOMPOK C</b></td></tr>';
            $q_mapel = $this->db->query("SELECT * FROM m_mapel WHERE kelompok = 'C' AND tambahan_sub = 'NO'")->result_array();
           ...
           foreach ($q_mapel as $i=>$m) {
                ...
                $npp = empty($nilai_pengetahuan[$idx]['predikat']) ? "-" : $nilai_pengetahuan[$idx]['predikat'];
                $npd = empty($nilai_pengetahuan[$idx]['desk']) ? "-" : "Capaian kompetensi sudah tuntas dengan predikat ".nilai_pre($nka);
                $nka = empty($nilai_keterampilan[$idx]['nilai']) ? "-" : $nilai_keterampilan[$idx]['nilai'];
                $nkp = empty($nilai_keterampilan[$idx]['predikat']) ? "-" : $nilai_keterampilan[$idx]['predikat'];
    ...
    public function index() {
        ...
        if ($wali['is_wali'] == true){
            $this->d['surat_izin'] = $this->db->query("SELECT *, b.nama as nama_siswa, a.id as id_surat, d.nama as guru, a.status as status_surat FROM m_surat_izin a
                                                        JOIN m_siswa b ON a.id_siswa = b.id
                                                        JOIN t_walikelas c ON a.id_walikelas = c.id
                                                        JOIN m_guru d ON c.id_guru = d.id
                                                        WHERE c.id_guru = ".$wali['id_walikelas']."")->result_array();

        }else{
            $this->d['surat_izin'] = $this->db->query("SELECT *, a.id as id_surat, d.nama as guru, a.status as status_surat FROM m_surat_izin a
                                                        JOIN m_siswa b ON a.id_siswa = b.id
                                                        JOIN t_walikelas c ON a.id_walikelas = c.id
                                                        JOIN m_guru d ON c.id_guru = d.id
    ...
    public function edit($id){
        $this->d['p'] = "edit";
        $this->d['data'] = $this->db->query("SELECT * FROM m_surat_izin a WHERE id ='$id'")->row();
        $this->load->view("template_utama", $this->d);
    }

    public function tambah(){
        $this->d['p'] = "tambah";
        // $this->d['r_guru'] = $this->db->query("SELECT * FROM t_walikelas a JOIN m_guru b ON a.id_guru = b.id")->result_array();
        $this->load->view("template_utama", $this->d);
    }

    public function detail($id){
        $this->d['p'] = "detail";
        $this->d['data'] = $this->db->query("SELECT *, d.nama as nama_guru, a.status as status_surat FROM m_surat_izin a 
                        JOIN m_siswa b ON a.id_siswa = b.id
                        JOIN t_walikelas c ON a.id_walikelas = c.id
                        JOIN m_guru d ON c.id_guru = d.id
                        WHERE a.id = '$id'")->row();
    ...
    public function simpan(){
        ...
        if($p['id'] != ""){
            // var_dump($p['id']);
            // die();
            $cek = $this->db->get_where('m_surat_izin', array('id'=>$p['id']))->num_rows();
            if ($cek > 0) {
            // $this->db->query("UPDATE m_guru SET nama = '".$p['nama']."', nip = '".$p['nip']."', is_bk = '".$p['isbk']."' WHERE id = '".$p['_id']."'");
                $this->db->query("UPDATE m_surat_izin SET  jenis = '".$p['jenis']."', surat = '".$p['surat']."', status = '".$p['status']."' WHERE id = '".$p['id']."' ");
            $d['data'] = 'Surat tersimpan';
            
            }else{
                $d['data'] = 'Tidak ditemukan';
            }
        }else{
            $wali = $this->db->query("SELECT c.id as walikelas FROM m_siswa a 
                                    JOIN t_kelas_siswa b ON b.id_siswa = a.id
                                    JOIN t_walikelas c ON c.id_kelas = b.id_kelas
                                    WHERE a.id=".$this->d['admkonid']."")->row();
    
            $this->db->query("INSERT INTO m_surat_izin (id_siswa, id_walikelas, jenis, surat) VALUES ('".$this->d['admkonid']."', '".$wali->walikelas."', '".$p['jenis']."', '".$p['surat']."')");
            $d['data'] = 'Surat tersimpan';
            
        }
        j($d);
        exit;
        $this->session->set_flashdata('k', '<div class="alert alert-success">Surat tersimpan</div>');
